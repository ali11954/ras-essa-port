...
def import_data():
    ...
    excel_path = r'D:\ghith\ras_essa_port\port.xlsx'
    ...
    try:
        ...
        df = pd.read_excel(excel_path, sheet_name='الكشف الكلي (9)', header=None)
        ...
        with app.app_context():
            ...
            # البيانات تبدأ من الصف 3 (index 2 في pandas)
            ...
            print("\n📋 توزيع الموظفين:")
            for i in range(1, 32):
                team = Team.query.filter_by(name=f'الفرقة {i}').first()
                if team:
                    count = Employee.query.filter_by(team_id=team.id).count()
                    if count > 0:
                        print(f"   الفرقة {i}: {count} موظف")
